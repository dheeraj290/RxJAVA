...
public class Conditionals implements Program {
	...
	public void run() {
		...
		//Observable<String> words = Observable.just("Some", "Other","there");
		...
		*/
		Observable<String> words = Observable.just("one", "way", "or", "another", "I'll", "learn", "RxJava")
				.zipWith(Observable.interval(100L, TimeUnit.MILLISECONDS), (x, y) -> x);
		
		//blockingSubscribePrint(words.takeUntil(interval)/*.delay(1L, TimeUnit.SECONDS)*/, "takeUntil");
		//blockingSubscribePrint(words.delay(800L, TimeUnit.MILLISECONDS).takeWhile(word -> word.length() > 2), "takeWhile");
		...
		blockingSubscribePrint(words.skipWhile(word -> word.length() >= 3), "skipWhile");
